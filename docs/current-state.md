...
M0 완료. M1 First Playable Loop 완료.
...
없음. M1 First Playable Loop 계획 `exec-plans/004-first-playable-loop.md`는 완료되었다.
...
M2 Change Request 범위를 논의한다. M2는 기존 M1 기능을 망가뜨리지 않고 작은 변경을 수행할 수 있는지 확인하는 단계다.

M1 기간 동안 hooks, MCP, custom skills, subagents, 별도 template 폴더 같은 하네스 확장은 추가하지 않았다.

M2를 시작하려면 기존 워크플로우 규칙에 따라 먼저 사용자 의도와 범위를 논의하고, 필요한 경우 design 기준 확인과 ExecPlan 절차를 따른다.
