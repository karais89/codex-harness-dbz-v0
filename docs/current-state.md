...
006 이후 `core-beliefs.md` 초안 자격 조건을 보강했다. `docs/design/core-beliefs.md`는 현재 `상태: 작성 전`이며 아직 실제 design 산출물이 아니다. 사용자 인터뷰와 공유된 이해 검증 후에만 `상태: 초안`으로 다시 작성할 수 있고, 사용자 명시 승인 전에는 `상태: 승인됨`으로 바꾸지 않는다. 이 상태에서는 M1 gameplay ExecPlan을 시작할 수 없다.

## 활성 계획

없음. Core Beliefs 작성 절차 보강 계획 `exec-plans/007-core-beliefs-authoring-flow.md`는 완료되었다.
...
M1을 시작하려면 먼저 `docs/design/core-beliefs.md` 작성을 위한 사용자 인터뷰와 공유된 이해 검증이 필요하다. 그 뒤 `core-beliefs.md`를 `상태: 초안`으로 작성하고, 사용자가 명시 승인해 `상태: 승인됨`으로 전환해야 한다. 그 다음 M1에 필요한 조건부 design 문서가 필요한지 판단하고, 필요한 경우 design 문서 생성 또는 갱신 절차를 진행한다. M1 ExecPlan 파일은 기존 결정대로 `exec-plans/004-first-playable-loop.md`를 사용하지만, 이번 작업에서는 생성하거나 수정하지 않는다.
...
- 006 이후 발견된 보완점에 따라 `core-beliefs.md`는 인터뷰와 공유된 이해 검증 전에는 design 산출물이 아니라는 규칙을 추가하고, 현재 상태를 `상태: 작성 전`으로 전환했다.

## 다음 단계

`docs/design/core-beliefs.md` 작성을 위한 사용자 인터뷰를 진행한다. 인터뷰 후 공유된 이해 검증에서 사용자가 의도와 맞다고 확인하면 `core-beliefs.md`를 `상태: 초안`으로 다시 작성한다. 승인 전에는 M1 First Playable Loop를 시작하지 않는다.

`core-beliefs.md`가 `상태: 승인됨`이 된 이후 M1에 필요한 조건부 design 문서가 필요한지 판단한다. 필요한 경우 새 design 문서는 `상태: 초안`으로 시작하고 사용자 승인 전에는 gameplay ExecPlan 기준으로 사용하지 않는다.

M1을 다시 시작하려면 새 워크플로우 규칙과 design 기준 규칙에 따라 먼저 사용자 의도와 범위를 인터뷰하고, 공유된 이해 검증 뒤 `exec-plans/004-first-playable-loop.md` 생성 여부를 확인한다.
...
- 게임 명세
- M1 범위 합의
- `docs/design/core-beliefs.md` 작성 인터뷰
- `docs/design/core-beliefs.md` 초안 작성
- `docs/design/core-beliefs.md` 사용자 승인
- M1에 필요한 조건부 design 문서 생성 또는 승인 여부 판단
- `exec-plans/004-first-playable-loop.md` 생성 또는 수정
- Unity MCP
- 사용자 정의 skill
- hook
- subagent
