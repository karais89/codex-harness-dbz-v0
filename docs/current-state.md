...
M0 완료. M1 First Playable Loop 완료. M2 Readability and Regression Check 완료.
...
없음. M2 Readability and Regression Check 계획 `exec-plans/008-m2-readability-regression-check.md`는 완료되었다.
...
M3 검증/테스트 루프 정착 범위를 논의한다. M3는 감이 아니라 반복 기준으로 검증 가능한지 확인하는 단계이며, 기존 로드맵 기준으로 수동 Play Mode 체크리스트와 가능한 경우 간단한 Edit Mode Test 1-3개를 검토한다.

M1과 M2 기간 동안 hooks, MCP, custom skills, subagents, 별도 template 폴더 같은 하네스 확장은 추가하지 않았다.

M3를 시작하려면 기존 워크플로우 규칙에 따라 먼저 사용자 의도와 범위를 논의하고, 필요한 경우 design 기준 확인과 ExecPlan 절차를 따른다.
