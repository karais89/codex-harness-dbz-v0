...
M1 gameplay loop 인터뷰와 공유된 이해 검증을 거쳐 `docs/design/gameplay-loop.md`를 `상태: 초안`으로 작성했다. 이 문서는 아직 승인 전이므로 M1 gameplay ExecPlan 기준으로 사용할 수 없다.
...
`docs/design/core-beliefs.md`는 승인됐다. `docs/design/gameplay-loop.md`는 초안 상태다. 해당 design 문서가 승인되기 전까지 M1 gameplay ExecPlan은 시작하지 않는다. M1 ExecPlan 파일은 기존 결정대로 `exec-plans/004-first-playable-loop.md`를 사용하지만, 이번 작업에서는 생성하거나 수정하지 않는다.
...
사용자가 `docs/design/gameplay-loop.md`를 검토하고 승인 여부를 결정한다. 승인하려면 사용자가 명시적으로 이 문서의 상태를 `승인됨`으로 바꾸라고 지시해야 한다.
