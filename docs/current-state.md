...
006 이후 `core-beliefs.md` 초안 자격 조건을 보강했다. 2026-05-19 사용자 인터뷰와 공유된 이해 검증을 거쳐 `docs/design/core-beliefs.md`를 `상태: 초안`으로 작성했다. 사용자 명시 승인 전에는 `상태: 승인됨`으로 바꾸지 않는다. 초안 상태에서는 M1 gameplay ExecPlan을 시작할 수 없다.
...
M1을 시작하려면 사용자가 `docs/design/core-beliefs.md`를 명시 승인해 `상태: 승인됨`으로 전환해야 한다. 그 다음 M1에 필요한 조건부 design 문서가 필요한지 판단하고, 필요한 경우 design 문서 생성 또는 갱신 절차를 진행한다. M1 ExecPlan 파일은 기존 결정대로 `exec-plans/004-first-playable-loop.md`를 사용하지만, 이번 작업에서는 생성하거나 수정하지 않는다.
...
사용자가 `docs/design/core-beliefs.md` 초안을 검토하고, 의도와 맞으면 명시적으로 `상태: 승인됨` 전환을 승인한다. 승인 전에는 M1 First Playable Loop를 시작하지 않는다.
