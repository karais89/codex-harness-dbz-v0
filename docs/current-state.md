...
006 이후 `core-beliefs.md` 초안 자격 조건을 보강했다. 2026-05-19 사용자 인터뷰와 공유된 이해 검증을 거쳐 `docs/design/core-beliefs.md`를 `상태: 초안`으로 작성했고, 이후 사용자가 명시 승인해 `상태: 승인됨`으로 전환했다. M1 gameplay ExecPlan을 시작하려면 M1의 player-facing gameplay 규칙을 정의하는 별도 조건부 design 문서도 필요하다.
...
`docs/design/core-beliefs.md`는 승인됐다. M1 First Playable Loop에는 조작, 이동, pickup, deliver, result, retry의 player-facing 규칙이 필요하므로 별도 조건부 design 문서가 필요하다. 해당 design 문서가 승인되기 전까지 M1 gameplay ExecPlan은 시작하지 않는다. M1 ExecPlan 파일은 기존 결정대로 `exec-plans/004-first-playable-loop.md`를 사용하지만, 이번 작업에서는 생성하거나 수정하지 않는다.
...
M1 First Playable Loop의 player-facing gameplay 규칙을 정의할 조건부 design 문서 범위를 사용자와 확인한다. 필요한 경우 새 design 문서는 `상태: 초안`으로 시작하고 사용자 승인 전에는 gameplay ExecPlan 기준으로 사용하지 않는다.

조건부 design 문서가 `상태: 승인됨`이 된 뒤에만 `exec-plans/004-first-playable-loop.md` 생성 여부를 확인한다.
...
- 게임 명세
- M1 범위 합의
- M1에 필요한 조건부 design 문서 생성 또는 승인
