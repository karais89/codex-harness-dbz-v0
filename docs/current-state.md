...
M1을 시작하려면 새 규칙에 따라 논의, `grill-me` 방식 인터뷰, 공유된 이해 검증, ExecPlan 생성 승인 절차를 먼저 진행한다. M1 ExecPlan 파일은 기존 결정대로 `exec-plans/004-first-playable-loop.md`를 사용한다.
...
M1을 다시 시작하려면 새 워크플로우 규칙에 따라 먼저 사용자 의도와 범위를 인터뷰하고, 공유된 이해 검증 뒤 `exec-plans/004-first-playable-loop.md` 생성 여부를 확인한다.
